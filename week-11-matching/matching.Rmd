...
x <- round(rnorm(100, 40, 10),0)
y <- round((x + runif(100, 0, 20)) * 5,1)

con <- data.frame(x1 = x,
                  y1 = y)

ggplot(con, aes(x=x1,y=y1)) +
    geom_point(size=3, colour="gray20") +
    geom_polygon(data = con[chull(x, y), ], color="red", fill="red", alpha = 0.15) +
    theme_minimal() +
    xlab("Age") +
    ylab("Income") +
    ylim(100,420) +
...
ggplot(con, aes(x=x1,y=y1)) +
    ...
    ylim(100,420) +
    theme(axis.title=element_text(size=16)) +
    geom_point(size=3, colour="gray20", aes(x=c(22),y=c(max(y1 + 20))))
```
...
ggplot(con, aes(x=x1,y=y1)) +
    ...
    theme(axis.title=element_text(size=16)) +
    geom_point(size=3, colour="gray20", aes(x=c(22),y=c(max(y1 + 20)))) +
    annotate("text", x = 33, y = 390, size=16, label = "WTF?", face="bold")
```
